...
String buildTimestamp() {
    if (!gps.date.isValid() || !gps.time.isValid()) {
        // GPS time not yet acquired; use a clearly invalid sentinel
...
void sendPosition() {
    ...
    http.setTimeout(5000);   // 5-second timeout so the loop is not blocked
    ...
    if (httpCode > 0) {
        ...
        // Negative codes are HTTPClient error constants, not HTTP status codes
...
void setup() {
    Serial.begin(115200);
    delay(500);   // Give the Serial Monitor time to open

    Serial.println("\n╔══════════════════════════════════╗");
    Serial.println("║  CFG Digital Twin — GPS Sender   ║");
    Serial.println("╚══════════════════════════════════╝");

    // UART1: GPS_RX_PIN reads data FROM the GPS (GPS TX line)
...
void loop() {
    // Continuously feed GPS NMEA bytes into TinyGPS++ parser
    ...
    if (now - lastSendTime >= SEND_INTERVAL_MS) {
        ...
        // WiFi guard — reconnect if dropped
        if (WiFi.status() != WL_CONNECTED) {
